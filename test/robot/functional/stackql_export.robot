...
Export User Space Table and then Access From RDBMSs Over Stackql Postgres Server
    ...
    ${result} =    Run Process
    ...    ${shellExe}     \-c    ${input}
    ...    stdout=${CURDIR}/tmp/Export-User-Space-Table-and-then-Access-From-RDBMSs-Over-Stackql-Postgres-Server-Drop.tmp
    ...    stderr=${CURDIR}/tmp/Export-User-Space-Table-and-then-Access-From-RDBMSs-Over-Stackql-Postgres-Server-Drop-stderr.tmp
    Should Be Equal    ${result.stdout}    OK
    ${ddlInputStr} =    Catenate
    ...    create table my_silly_export_table_two(id int, name text, magnitude numeric);
    ${posixInput} =     Catenate
    ...
    ${result} =    Run Process
    ...    ${shellExe}     \-c    ${input}
    ...    stdout=${CURDIR}/tmp/Export-User-Space-Table-and-then-Access-From-RDBMSs-Over-Stackql-Postgres-Server-Recreate.tmp
    ...    stderr=${CURDIR}/tmp/Export-User-Space-Table-and-then-Access-From-RDBMSs-Over-Stackql-Postgres-Server-Recreate-stderr.tmp
    Should Be Equal    ${result.stdout}    OK
    ${ddlInputStr} =    Catenate
