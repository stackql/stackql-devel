...
type (
	...
	// With represents a WITH clause (CTE - Common Table Expression)
	With struct {
		Recursive bool
...
type (
	...
	// FrameClause represents a frame clause in a window specification
	FrameClause struct {
	...
	// FramePoint represents a frame boundary point
	FramePoint struct {
...
func (node *CommonTableExpr) Format(buf *TrackedBuffer) {
	...
	buf.astPrintf(node, " as %v", node.Subquery)
}
...
func (node *FramePoint) Format(buf *TrackedBuffer) {
	switch node.Type {
	case UnboundedPrecedingStr, CurrentRowStr, UnboundedFollowingStr:
		buf.WriteString(node.Type)
	default:
		buf.astPrintf(node, "%v %s", node.Expr, node.Type)
	}
}
