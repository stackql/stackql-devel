...
%token <bytes> OVER ROWS RANGE UNBOUNDED PRECEDING FOLLOWING CURRENT ROW

// CTE tokens
%token <bytes> RECURSIVE
...
%token <bytes> NTH_VALUE NTILE OF PERCENT_RANK RANK ROW_NUMBER SYSTEM WINDOW
%token <bytes> ACTIVE ADMIN BUCKETS CLONE COMPONENT DEFINITION ENFORCED EXCLUDE GEOMCOLLECTION GET_MASTER_PUBLIC_KEY HISTOGRAM HISTORY
...
%token <bytes> NESTED NETWORK_NAMESPACE NOWAIT NULLS OJ OLD OPTIONAL ORDINALITY ORGANIZATION OTHERS PATH PERSIST PERSIST_ONLY PRIVILEGE_CHECKS_USER PROCESS
...
%token <bytes> THREAD_PRIORITY TIES VCPU VISIBLE
...
| WITH cte_list SELECT comment_opt select_options select_expression_list from_opt where_expression_opt group_by_opt having_opt
  {
    sel := NewSelect(Comments($4), $6/*SelectExprs*/, $5/*options*/, $7/*from*/, NewWhere(WhereStr, $8), GroupBy($9), NewWhere(HavingStr, $10))
    sel.With = &With{Recursive: false, CTEs: $2}
    $$ = sel
  }
| WITH RECURSIVE cte_list SELECT comment_opt select_options select_expression_list from_opt where_expression_opt group_by_opt having_opt
  {
    sel := NewSelect(Comments($5), $7/*SelectExprs*/, $6/*options*/, $8/*from*/, NewWhere(WhereStr, $9), GroupBy($10), NewWhere(HavingStr, $11))
    sel.With = &With{Recursive: true, CTEs: $3}
    $$ = sel
  }
...
/*
  Window function OVER clause and window specifications
*/
over_clause_opt:
  {
    $$ = nil
...
| OVER openb closeb
  {
    $$ = &OverClause{WindowSpec: &WindowSpec{}}
  }
| OVER sql_id
  {
...
window_spec:
  partition_by_opt order_by_opt frame_clause_opt
  {
    $$ = &WindowSpec{PartitionBy: $1, OrderBy: $2, Frame: $3}
  }
...
| ROWS frame_point
  {
    $$ = &FrameClause{Unit: RowsStr, Start: $2}
  }
| ROWS BETWEEN frame_point AND frame_point
  {
    $$ = &FrameClause{Unit: RowsStr, Start: $3, End: $5}
  }
| RANGE frame_point
  {
    $$ = &FrameClause{Unit: RangeStr, Start: $2}
  }
| RANGE BETWEEN frame_point AND frame_point
  {
    $$ = &FrameClause{Unit: RangeStr, Start: $3, End: $5}
  }

frame_point:
  UNBOUNDED PRECEDING
  {
    $$ = &FramePoint{Type: UnboundedPrecedingStr}
  }
| UNBOUNDED FOLLOWING
  {
    $$ = &FramePoint{Type: UnboundedFollowingStr}
  }
| CURRENT ROW
  {
    $$ = &FramePoint{Type: CurrentRowStr}
  }
...
// CTE (Common Table Expression) rules
with_clause_opt:
  {
    $$ = nil
  }
...
cte:
  ...
  {
    $$ = &CommonTableExpr{Name: $1, Subquery: &Subquery{Select: $4}}
  }
| table_id openb column_list closeb AS openb select_statement closeb
  {
    $$ = &CommonTableExpr{Name: $1, Columns: $3, Subquery: &Subquery{Select: $7}}
  }
