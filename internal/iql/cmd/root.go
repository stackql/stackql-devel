...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&runtimeCtx.KeyFilePath, dto.KeyFilePathKey, config.GetDefaultKeyFilePath(), fmt.Sprintf("key filename"))
	rootCmd.PersistentFlags().StringVar(&runtimeCtx.KeyFileType, dto.KeyFileTypeKey, dto.AuthServiceAccountStr, fmt.Sprintf("key file type, permitted values: -'%s' - '%s'", dto.AuthServiceAccountStr, dto.AuthApiKeyStr))
