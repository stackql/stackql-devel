...
func (se snowflakeTCPEngine) getNextGenerationID() (int, error) {
	var retVal int
	//nolint:lll // long SQL query
	_, err := se.db.Exec(`INSERT INTO "__iql__.control.generation" (generation_description, created_dttm) VALUES ('', current_timestamp) RETURNING iql_generation_id`)
	return retVal, err
}
...
func (se snowflakeTCPEngine) getNextProviderGenerationID(providerName string) (int, error) {
	var retVal int
	//nolint:lll // long SQL query
	_, err := se.db.Exec(`INSERT INTO "__iql__.control.discovery_generation" (discovery_name, created_dttm) VALUES ($1, current_timestamp) RETURNING iql_discovery_generation_id`, providerName)
	return retVal, err
}
...
func (se snowflakeTCPEngine) getNextSessionID(generationID int) (int, error) {
	var retVal int
	//nolint:lll // long SQL query
	_, err := se.db.Exec(`INSERT INTO "__iql__.control.session" (iql_generation_id, created_dttm) VALUES ($1, current_timestamp) RETURNING iql_session_id`, generationID)
