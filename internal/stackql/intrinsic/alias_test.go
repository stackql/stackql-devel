package intrinsic //nolint:testpackage // tests unexported alias plumbing
...
func TestAliasProviderRequiresOptIn(t *testing.T) {
	withUnstable(t, false)
	for _, name := range []string{DynamicProviderName, IaCProviderName} {
		if _, ok := aliasProvider(name); ok {
			t.Errorf("aliasProvider(%q) = true without the unstable opt-in", name)
		}
		if IsProvider(name) {
			t.Errorf("IsProvider(%q) = true without the unstable opt-in", name)
		}
	}
}

func TestAliasProviderCanonicalises(t *testing.T) {
	withUnstable(t, true)
	cases := map[string]string{
		"stackql_dynamic":   DynamicProviderName,
		"STACKQL_DYNAMIC":   DynamicProviderName,
		"  stackql_iac  ":   IaCProviderName,
		"StAcKqL_iAc":       IaCProviderName,
		"stackql_preview":   "",
		"stackql_dynamics":  "",
		"stackql_iac_extra": "",
	...
	for input, want := range cases {
		got, ok := aliasProvider(input)
		if want == "" {
			if ok {
				t.Errorf("aliasProvider(%q) = %q, want no match", input, got)
			}
			continue
		}
		if !ok || got != want {
			t.Errorf("aliasProvider(%q) = %q, %v; want %q, true", input, got, ok, want)
		}
	}
}

func TestAliasServices(t *testing.T) {
	withUnstable(t, true)
	dynamic := aliasServices(DynamicProviderName)
	if len(dynamic) != 1 || dynamic[0] != graphService {
		t.Errorf("aliasServices(dynamic) = %v, want [%s]", dynamic, graphService)
	}
	iac := aliasServices(IaCProviderName)
	if len(iac) != 2 || iac[0] != blueprintService || iac[1] != convergeService {
		t.Errorf("aliasServices(iac) = %v, want [%s %s]", iac, blueprintService, convergeService)
	}
}
