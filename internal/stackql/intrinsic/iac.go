...
// stackql_iac presents omnisdk's converge run as a relation, and the blueprints
// that render one as a catalogue. A run is issued as an imperative - these
...
// iacSelectFunc routes a SELECT over stackql_iac. Opening the plan performs the
// run, so a select over the converge relation is a mutation in every sense
// except its syntax.
func iacSelectFunc(
	ctx queryContext,
	...
	if strings.EqualFold(service, blueprintService) {
		return blueprintSelectFunc(ctx, node, resource)
	}
	if !strings.EqualFold(service, convergeService) || !strings.EqualFold(resource, convergeRelation) {
		return nil, false
	}
	relation := fmt.Sprintf("%s.%s.%s", IaCProviderName, convergeService, convergeRelation)
	params, refusal := aliasPredicates(node, relation)
	if refusal != nil {
		return refusal, true
	}
	...
	return func() internaldto.ExecutorOutput {
		...
		plan, planErr := omnisdk.Converge(
			...
			aliasArgs(ctx, convergeCloud(resources), params))
		if planErr != nil {
...
func blueprintResources(handle string, params map[string]string) ([]omnisdk.ManagedResource, error) {
	...
	if !ok {
		return nil, fmt.Errorf(
			...
			handle, IaCProviderName, blueprintService)
	}
...
func blueprintSelectFunc(
	_ queryContext,
	_ *sqlparser.Select,
	resource string,
) (func() internaldto.ExecutorOutput, bool) {
	return refuse(fmt.Errorf(
		"'%s.%s.%s' renders a deployment rather than rows; "+
			"apply it with SELECT ... FROM %s.%s.%s WHERE %s = '%s' AND %s = '<name>'",
		IaCProviderName, blueprintService, resource,
		IaCProviderName, convergeService, convergeRelation,
