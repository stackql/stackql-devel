...
// docRoot is the bundle's own directory inside stackql's registry root. The
...
func docRoot(ctx queryContext, bundle string) (string, error) {
	root := filepath.Join(ctx.GetRuntimeContext().ApplicationFilesRootPath, "src", bundleDir(bundle))
