...
// stackql_dynamic presents omnisdk's multi-exchange query as a single relation.
...
// dynamicSelectFunc routes a SELECT over stackql_dynamic.graph.query.
func dynamicSelectFunc(
	ctx queryContext,
	node *sqlparser.Select,
	service, resource string,
) (func() internaldto.ExecutorOutput, bool) {
	if !strings.EqualFold(service, graphService) || !strings.EqualFold(resource, graphRelation) {
		return nil, false
	}
	relation := fmt.Sprintf("%s.%s.%s", DynamicProviderName, graphService, graphRelation)
	...
	return func() internaldto.ExecutorOutput {
		...
		plan, planErr := omnisdk.NewGraphQuery(
			registryRoot(ctx), graph, aliasArgs(ctx, graphCloud(graph.Addresses()), params))
		if planErr != nil {
