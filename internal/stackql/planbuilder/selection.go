...
func (p *primitiveGenerator) analyzeUnarySelection(
	...
	if len(cols) == 0 {
		...
		for _, v := range colNames {
			...
			existingParams[v] = struct{}{}
		}
		for k, _ := range method.GetRequiredParameters() {
			if _, ok := existingParams[k]; ok {
				continue
			}
			cols = append(cols, parserutil.NewUnaliasedColumnHandle(k))
			existingParams[k] = struct{}{}
		}
	}
