...
def build_stackql(verbose :bool) -> int:
    ...
    return subprocess.call(
        f'go build {"-x -v" if verbose else ""} --tags "json1 sqlite_stackql" -ldflags "-X github.com/stackql/stackql/internal/stackql/cmd.BuildMajorVersion={os.environ.get("BUILDMAJORVERSION")} '
...
def unit_test_stackql(verbose :bool) -> int:
    return subprocess.call(
        f'go test -timeout 1200s {"-v" if verbose else ""} --tags "json1 sqlite_stackql"  ./...',
        shell=True
    )
